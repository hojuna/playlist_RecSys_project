...
class DataPreprocess:
    ...
    def select_columns_and_remove_duplicates(self, df_playlist: pd.DataFrame) -> pd.DataFrame:
    ...
    def remove_missing_values(self, df_playlist: pd.DataFrame) -> pd.DataFrame:
        # **누락된 값이 있는 행 제거**
        df_playlist.dropna(subset=['trackname', 'artistname'], inplace=True)
        return df_playlist

    def filter_songs_by_frequency(self, df_playlist: pd.DataFrame, min_frequency: int) -> pd.DataFrame:
        # ------------------ 노래 빈도 기반 필터링 시작 ------------------
    ...
    def assign_song_index(self, df_playlist: pd.DataFrame) -> pd.DataFrame:
    ...
    def calculate_user_song_id_count(self, df_playlist: pd.DataFrame) -> pd.DataFrame:
        # 각 유저가 들은 노래 수 계산
    ...
    def filter_users_by_song_count(self, user_song_count: pd.DataFrame, lower_percent: float, mean_percent: float) -> pd.DataFrame:
        # 평균 및 표준편차 계산
    ...
    def assign_user_index(self, df_playlist: pd.DataFrame, filtered_users: pd.DataFrame) -> pd.DataFrame:
        # 필터링된 유저만 남기기
    ...
    def select_user_song_columns(self, df_playlist: pd.DataFrame) -> pd.DataFrame:
        # 필요한 컬럼만 선택
        df_user_songs = df_playlist[['user_index', 'song_index']].drop_duplicates().reset_index(drop=True)
        return df_user_songs

    def create_sparse_matrix(self, df_user_songs: pd.DataFrame) -> csr_matrix:
        # 희소 행렬 생성
    ...
    def save_sparse_matrix_and_dataframes(self, sparse_matrix: csr_matrix, df_playlist: pd.DataFrame, min_frequency: int, save_path: str):
        # 희소 행렬 저장
    ...
    def print_sparse_matrix_info(self, sparse_matrix: csr_matrix):
    ...
    def data_clean_up(self, df_playlist: pd.DataFrame) -> pd.DataFrame:
    ...
    def main(self, file_path: str, min_frequency: int, lower_percent: float, mean_percent: float, save_path: str):


        df_playlist = self.read_csv_file(file_path)

        df_playlist = self.select_columns_and_remove_duplicates(df_playlist)
        df_playlist = self.remove_missing_values(df_playlist)


        df_playlist = self.filter_songs_by_frequency(df_playlist, min_frequency)
        df_playlist = self.assign_song_index(df_playlist)

        
        user_song_count = self.calculate_user_song_count(df_playlist)
        filtered_users = self.filter_users_by_song_count(user_song_count, lower_percent, mean_percent)
        df_playlist = self.assign_user_index(df_playlist, filtered_users)
        df_user_songs = self.select_user_song_columns(df_playlist)
        sparse_matrix = self.create_sparse_matrix(df_user_songs)
        self.save_sparse_matrix_and_dataframes(sparse_matrix, df_playlist, min_frequency, save_path)
        self.print_sparse_matrix_info(sparse_matrix)
...
if __name__ == "__main__":
    ...
    data_preprocess.main(file_path, min_frequency, lower_percent, mean_percent, save_path)
